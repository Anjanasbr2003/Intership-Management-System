  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col gap-2 pointer-events-none max-w-sm w-full sm:w-auto">
      {toasts.map((t) => (
        <div
          key={t.id}
          className="card-elevated shadow-lg pointer-events-auto flex items-center justify-between gap-3 px-4 py-3 rounded-2xl text-text-primary text-xs sm:text-sm animate-toast"
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            {t.type === 'info' && <Info className="w-4 h-4 text-info shrink-0" />}
            <span className="font-medium text-xs sm:text-sm tracking-tight text-text-primary">{t.message}</span>
          </div>
          <button
            onClick={() => removeToast(t.id)}
            className="transition-transform active:scale-95 text-text-muted hover:text-text-primary p-1 rounded-full hover:bg-surface-hover transition-colors"
